CREATE TABLE custom_pizzas (
  ...
  name varchar,
  dough varchar DEFAULT 'Традиционное',
  size varchar DEFAULT '20 см',
...
CREATE TABLE custom_pizza_extra_ingredients (
  id bigserial PRIMARY KEY,
  custom_pizza_id bigint NOT NULL,
  ingredient_id bigint NOT NULL,
  ...
  created_at timestamp DEFAULT NOW(),
  FOREIGN KEY (custom_pizza_id) REFERENCES custom_pizzas(id) ON DELETE CASCADE,
  FOREIGN KEY (ingredient_id) REFERENCES toppings(id) ON DELETE CASCADE
);
